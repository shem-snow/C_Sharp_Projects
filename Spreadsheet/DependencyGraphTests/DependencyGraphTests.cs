...
namespace DevelopmentTests
{
    ...
    public class DependencyGraphTest
    {
        ...
        public void SimpleEmptyRemoveTest()
        {
            ...
            depGraph.AddDependency("x", "y");
            Assert.AreEqual(1, depGraph.Size);
            depGraph.RemoveDependency("x", "y");
            Assert.AreEqual(0, depGraph.Size);
        }
        ...
        public void EmptyEnumeratorTest()
        {
            DependencyGraph depGraph = new DependencyGraph();
            depGraph.AddDependency("x", "y");
            IEnumerator<string> e1 = depGraph.GetDependees("y").GetEnumerator();
            Assert.IsTrue(e1.MoveNext());
            Assert.AreEqual("x", e1.Current);
            IEnumerator<string> e2 = depGraph.GetDependents("x").GetEnumerator();
            Assert.IsTrue(e2.MoveNext());
            Assert.AreEqual("y", e2.Current);
            depGraph.RemoveDependency("x", "y");
            Assert.IsFalse(depGraph.GetDependees("y").GetEnumerator().MoveNext());
            Assert.IsFalse(depGraph.GetDependents("x").GetEnumerator().MoveNext());
        }


        /// <summary>
        ///Replace on an empty DG shouldn'depGraph fail
        ///</summary>
        [TestMethod()]
        ...
        public void SizeTest()
        {
            ...
            Assert.AreEqual(4, depGraph.Size);
        }
        ...
        {
            DependencyGraph depGraph = new DependencyGraph();
            depGraph.AddDependency("x", "b");
            depGraph.AddDependency("a", "z");
            depGraph.ReplaceDependents("b", new HashSet<string>());
            depGraph.AddDependency("y", "b");
            depGraph.ReplaceDependents("a", new HashSet<string>() { "c" });
            depGraph.AddDependency("w", "d");
            depGraph.ReplaceDependees("b", new HashSet<string>() { "a", "c" });
            depGraph.ReplaceDependees("d", new HashSet<string>() { "b" });

            IEnumerator<string> e = depGraph.GetDependees("a").GetEnumerator();
            Assert.IsFalse(e.MoveNext());

            e = depGraph.GetDependees("b").GetEnumerator();
            Assert.IsTrue(e.MoveNext());
            String s1 = e.Current;
            Assert.IsTrue(e.MoveNext());
            String s2 = e.Current;
            ...
            e = depGraph.GetDependees("c").GetEnumerator();
            Assert.IsTrue(e.MoveNext());
            Assert.AreEqual("a", e.Current);
            Assert.IsFalse(e.MoveNext());

            e = depGraph.GetDependees("d").GetEnumerator();
            Assert.IsTrue(e.MoveNext());
            Assert.AreEqual("b", e.Current);
            Assert.IsFalse(e.MoveNext());
        }
        ...
        [TestMethod()]
        public void GetDependentsAndDependeesOfANonExistingNode()
        {
            ...
            // make sure it doesn'depGraph break when I try to fetch non-existent nodes.
            Assert.IsFalse(depGraph.HasDependees("x"));
            Assert.IsFalse(depGraph.HasDependents("y"));

        }

        [TestMethod()]
        public void ReplaceNonExistantNodesJustCreatesThem()
        {
            // make an empty depGraph
            DependencyGraph depGraph = new DependencyGraph();
            Assert.AreEqual(0, depGraph.Size);

            // replace "nothing" with something
            HashSet<string> dependents = new();
            dependents.Add("epstein");
            dependents.Add("didn'depGraph");
            dependents.Add("kill");
            dependents.Add("himself");
            depGraph.ReplaceDependents("jefferey", dependents);

            // see if it's filled up
            Assert.AreEqual(4, depGraph.Size);
            Assert.IsTrue(depGraph.HasDependents("jefferey"));
            Assert.IsTrue(depGraph.HasDependees("epstein"));
            Assert.IsTrue(depGraph.HasDependees("didn'depGraph"));
            Assert.IsTrue(depGraph.HasDependees("kill"));
            Assert.IsTrue(depGraph.HasDependees("himself"));
        }

        [TestMethod()]
        public void MoreReplaceOfEmpty()
        {
            // make an empty depGraph
            DependencyGraph depGraph = new DependencyGraph();
            Assert.AreEqual(0, depGraph.Size);
            ...
            dependees.Add("epstein");
            dependees.Add("didn'depGraph");
            dependees.Add("kill");
            dependees.Add("himself");
            depGraph.ReplaceDependees("jefferey", dependees);

            // see if it's filled up
            Assert.AreEqual(4, depGraph.Size);
            Assert.IsTrue(depGraph.HasDependees("jefferey"));
            Assert.IsTrue(depGraph.HasDependents("epstein"));
            Assert.IsTrue(depGraph.HasDependents("didn'depGraph"));
            Assert.IsTrue(depGraph.HasDependents("kill"));
            Assert.IsTrue(depGraph.HasDependents("himself"));
        }

        [TestMethod()]
        public void CanGetThisNode()
        {
            DependencyGraph depGraph = new DependencyGraph();
            depGraph.AddDependency("f", "g");
            depGraph.AddDependency("h", "i");

            Assert.AreEqual(0, depGraph["f"]);
            Assert.AreEqual(0, depGraph["h"]);

            depGraph.AddDependency("bill", "cosby");
            depGraph.AddDependency("yo", "mama");
            depGraph.AddDependency("bill", "nye the science guy");
            depGraph.AddDependency("bill", "y");

            Assert.AreNotEqual(3, depGraph["bill"]);
            Assert.AreEqual(0, depGraph["yo"]);

            depGraph.RemoveDependency("bill", "nye the science guy");

            Assert.AreEqual(0, depGraph["bill"]);

        }

        [TestMethod()]
        public void AddingRepeatedDependanciesDoesNotAdd()
        {
            DependencyGraph depGraph = new DependencyGraph();
            depGraph.AddDependency("a", "b");
            depGraph.AddDependency("a", "b");
            ...
            Assert.AreEqual(1, depGraph.Size);
            Assert.IsTrue(depGraph.HasDependents("a"));
            Assert.IsTrue(depGraph.HasDependees("b"));
        }

        [TestMethod()]
        public void GetOnEmptyReturnsEmpty()
        {
            DependencyGraph depGraph = new DependencyGraph();
            depGraph.AddDependency("a", "b");

            Assert.IsFalse(depGraph.GetDependees("a").GetEnumerator().MoveNext());
            Assert.IsFalse(depGraph.GetDependents("b").GetEnumerator().MoveNext());
        }

        [TestMethod()]
        public void GetNonExistantReturnsEmpty()
        {
            DependencyGraph depGraph = new DependencyGraph();

            Assert.IsFalse(depGraph.GetDependees("d").GetEnumerator().MoveNext());
            Assert.IsFalse(depGraph.GetDependents("d").GetEnumerator().MoveNext());
        }
        ...
        /// <summary>
        ///Removing from an empty DG shouldn'depGraph fail
        ///</summary>
        [TestMethod(), Timeout(2000)]
        [TestCategory("5")]
        ...
        // ********************** Making Sure that Static Variables Weren'depGraph Used ****************** //
        ///<summary>
        ...
        public void TestAddRemoveIndexer()
        {
            ...
            depGraph.AddDependency("c", "b");
            depGraph.RemoveDependency("a", "d");
            depGraph.AddDependency("e", "b");
            depGraph.AddDependency("b", "d");
            depGraph.RemoveDependency("e", "b");
            depGraph.RemoveDependency("x", "y");
            Assert.AreEqual(2, depGraph["b"]);
        }
        ...
        public void StressTest8()
        {
            ...
            for (int i = 0; i < SIZE; i++)
            {
                ...
                //Console.WriteLine(depGraph.GetDependees(letters[i]);
                Assert.IsTrue(dees[i].SetEquals(new HashSet<string>(t.GetDependees(letters[i]))));
            }
        }
