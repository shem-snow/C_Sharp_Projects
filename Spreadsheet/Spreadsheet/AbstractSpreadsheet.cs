...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        protected IEnumerable<string> GetCellsToRecalculate(string name)
        {
            ...
            // adds each cell to the unordered list, then the ordered list (but in order).
        ...
        /// 
        /// Before a cell is recalculated, it must return a list of all cells that depend on it
        /// and therefore must be recalculated as well.
        /// 
        /// This method finds all of those dependencies then determines in which order they should be recalculated.
        /// Note that it's really important to calculate them in the right order to avoid reculculating multiple times.
        /// 
        /// </summary>
        /// <param name="start"> Temporary variable to hold the name of each dependency during enumeration.</param>
        /// <param name="name"> The cell name that is being recalculated.</param>
        /// <param name="visited"> HashSet<string> that stores all the cell names that were visited</param>
        /// <param name="changed"> List<string> that stores values that were changed</param>
        /// <returns></returns>
        /// <exception cref="CircularException"></exception>
        private void Visit(string start, string name, ISet<string> visited, LinkedList<string> changed)
        {
            // The cell being recalculated needs to be put in the "visited" list.
            visited.Add(name);

            // For each of its dependencies
            foreach (string n in GetDirectDependents(name))
            {
                // Throw if it depends on itself (it's impossible to calculate a circular dependency).
                if (n.Equals(start))
                {
                    throw new CircularException();
                }
                // If the dependecy is not added the the "visited" list, then recursively call
                // this method.
            ...
            // The end result of the foreach loop is that every direct and indirect dependency will
            // exist in the "visited" list in the order that they were encountered during recursion.
            // This order is opposite of what we want in order to avoid 'duplicate calculations'
            //     (Think of the dependencies as a rooted tree.
            //      We should start at the leaves and work UP that tree).

            // Which is why our recursion's base case is:
            changed.AddFirst(name);

            // This line will be executed only at a 'bottom'/least encountered dependency.
            // And so the current "name" cell should be calculated first in order to avoid 'duplicate calculations'.
        }
